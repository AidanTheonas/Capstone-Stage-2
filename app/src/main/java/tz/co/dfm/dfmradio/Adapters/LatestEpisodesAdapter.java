...
public class LatestEpisodesAdapter
    extends RecyclerView.Adapter<LatestEpisodesAdapter.LatestEpisodeViewHolder>
    implements Parcelable {
  public static final int SHOWS_EPISODE_FRAGMENT = 1;
  public static final int SEARCH_ACTIVITY = 2;
  public static final Creator<LatestEpisodesAdapter> CREATOR =
      new Creator<LatestEpisodesAdapter>() {
        @Override
  ...
  public void onBindViewHolder(@NonNull LatestEpisodeViewHolder holder, int position) {
    ...
    if (!show.getEpisodeThumbnail().trim().equals("")) {
      Picasso.get().load(show.getEpisodeThumbnail()).into(holder.ivShowEpisodeImage);
    }
