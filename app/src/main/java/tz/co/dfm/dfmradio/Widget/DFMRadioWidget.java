...
public class DFMRadioWidget extends AppWidgetProvider {
    ...
    private static RemoteViews generateViews(Context context) {
        RemoteViews remoteViews = new RemoteViews(context.getPackageName(), R.layout.widget_default_view);
        ...
        PendingIntent playEpisodePendingIntent = PendingIntent.getActivity(context, 0,
                playEpisodeIntent, PendingIntent.FLAG_UPDATE_CURRENT);
        remoteViews.setPendingIntentTemplate(R.id.gv_widget_favorite_episodes, playEpisodePendingIntent);

        //Open main activity so user can choose episodes to add
        Intent addEpisodesIntent = new Intent(context, MainActivity.class);
        PendingIntent addEpisodesPendingIntent = PendingIntent.getActivity(context, 0,
                addEpisodesIntent, PendingIntent.FLAG_UPDATE_CURRENT);
    ...
    public static void updateDFMRadioWidget(Context context, AppWidgetManager manager, int[] widgetsIds) {
